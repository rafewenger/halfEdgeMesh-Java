...
public abstract class HalfEdgeBase {
	 ...
	 *  - Runs in time proportional to the number of half edges around edge.
	 */
	protected HalfEdgeBase PrevHalfEdgeAroundEdge() throws Exception
	{
		// Cannot have more than max_num half edges around an edge.
		int max_numh =
	...
	public float [] ComputeMidpointCoord()
	{
		float [] midpoint_coord = new float[VertexBase.DIMENSION];
		
		VertexBase vto = ToVertex();
		VertexBase vfrom = FromVertex();
		
		for (Integer d = 0; d < VertexBase.Dimension(); d++) 
		{ midpoint_coord[d] = (vto.Coord(d) + vfrom.Coord(d))/2.0f; };
